...
- [ ] 2.4 Update `capture(directory)` to use position-based naming: `A_<ts>.jpg` (left canvas) / `D_<ts>.jpg` (right canvas) based on current camera-to-canvas mapping
...
- [ ] 2.7 Add `swap_cameras()` method that reverses the camera-to-canvas mapping
- [ ] 2.8 Add `cameras_swapped` signal to notify GUI when mapping changes
- [ ] 2.9 Maintain `_camera_mapping` list to track which pipeline index maps to each canvas position

## 3. Dual-camera GUI

- [x] 3.1 Refactor `MainWindow` to accept `DualCameraManager` instead of a single `CameraPipeline`
- [ ] 3.2 Update two side-by-side `_PreviewWidget` instances with position labels: "Left (A)" and "Right (D)"
...
- [ ] 3.7 Add "Swap Cameras" button that calls `DualCameraManager.swap_cameras()`
- [ ] 3.8 Handle `cameras_swapped` signal: stop pipelines, swap window handle assignments, restart pipelines
